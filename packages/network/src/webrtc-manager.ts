...
export class WebRTCManager extends EventEmitter {
  ...
  constructor(config?: Partial<RTCConfig>) {
    ...
    // Initialize logger with robust fallback handling
  ...
  async createPeerConnection(
    ...
    try {
      ...
      // Store connection in map before creating data channels
   ...
   */ async createOffer(
    ...
    try {
      console.log("createOffer: Getting connection for", connectionId);
      const wrapper = this.getConnection(connectionId);
      console.log("createOffer: Got wrapper", wrapper ? "exists" : "null");

      console.log("createOffer: Calling wrapper.connection.createOffer");
      const offer = await wrapper.connection.createOffer(options);
      console.log("createOffer: Got offer", offer);

      console.log("createOffer: Setting local description");
      await wrapper.connection.setLocalDescription(offer);
      console.log("createOffer: Set local description");
      ...
      console.log("createOffer: Error occurred", error);
      this.logger.error("Failed to create offer", error);
      // Temporarily throw the original error to see what it is
      throw error;
      /*
      throw new NetworkError("Failed to create offer", "OFFER_CREATE_FAILED", {
        connectionId,
        error,
      });
      */
  ...
  async setRemoteDescription(
    ...
    try {
      ...
      this.logger.debug("Remote description set", {
        connectionId,
    ...
    try {
      ...
      if (!dataChannel) {
        throw new NetworkError(
          ...
          "DATACHANNEL_NOT_FOUND",
          { connectionId, channelLabel }
        );
      }
      ...
      throw new NetworkError("Failed to send data", "DATA_SEND_FAILED", {
        connectionId,
        channelLabel,
        error,
      });
    ...
    try {
      ...
      for (const [label, channel] of wrapper.dataChannels) {
        try {
          ...
          this.logger.debug(
            "Error closing data channel (expected during cleanup)",
            { connectionId, label, error: err }
          );
        }
      }
      ...
      try {
        ...
        this.logger.debug(
          "Error closing peer connection (expected during cleanup)",
      ...
      this.logger.warn("Error during connection cleanup", {
        connectionId,
        error,
      });
    }
  ...
  getConnectionInfo(connectionId: string): ConnectionInfo | null {
    ...
    return {
      ...
      createdAt: new Date(
        wrapper.lastActivity.getTime() - wrapper.statistics.connectionDuration
      ),
  ...
  private getConnection(connectionId: string): PeerConnectionWrapper {
    const wrapper = this.connections.get(connectionId);
    if (!wrapper) {
      throw new NetworkError("Connection not found", "CONNECTION_NOT_FOUND", {
        connectionId,
      });
    }
    return wrapper;
  ...
  private calculateConnectionQuality(wrapper: PeerConnectionWrapper) {
    ...
    const packetLossRate =
      statistics.packetsLost / (statistics.packetsSent || 1);
  ...
  private processRTCStats(
    ...
    wrapper.statistics.connectionDuration =
      Date.now() - wrapper.lastActivity.getTime();

    for (const stat of stats.values()) {
      if (stat.type === "inbound-rtp") {
        wrapper.statistics.bytesReceived =
          stat.bytesReceived || wrapper.statistics.bytesReceived;
        wrapper.statistics.packetsReceived =
          stat.packetsReceived || wrapper.statistics.packetsReceived;
        wrapper.statistics.packetsLost =
          stat.packetsLost || wrapper.statistics.packetsLost;
      } else if (stat.type === "outbound-rtp") {
        wrapper.statistics.bytesSent =
          stat.bytesSent || wrapper.statistics.bytesSent;
        wrapper.statistics.packetsSent =
          stat.packetsSent || wrapper.statistics.packetsSent;
      } else if (stat.type === "candidate-pair" && stat.state === "succeeded") {
        wrapper.statistics.averageLatency =
          stat.currentRoundTripTime || wrapper.statistics.averageLatency;
      }
    }
  }
  ...
  private initializeLogger(): any {
    try {
      // Try to create logger normally
      if (typeof Logger !== "undefined" && Logger.createLogger) {
        const logger = Logger.createLogger("WebRTCManager");
        if (logger) return logger;
      }
    } catch (error) {
      // Logger might not be available or might throw
      console.warn("WebRTCManager logger creation failed:", error);
    }

    // Check if we're in a test environment with a mock logger available
    if ((global as any).__mockLogger) {
      return (global as any).__mockLogger;
    }

    // Last resort fallback - create a basic logger interface
    const fallbackLogger = {
      ...
      stream: () => ({ write: () => {} }),
      logConnection: (...args: any[]) => {},
      logSession: (...args: any[]) => {},
      logAuth: (...args: any[]) => {},
      logNetworkEvent: (...args: any[]) => {},
      logMediaEvent: (...args: any[]) => {},
      logPerformance: (...args: any[]) => {},
    };

    console.warn("Using fallback logger in WebRTCManager");
