...
// Import socket.io-client to get the mocked version
  ...
  describe('WebRTCManager', () => {
    ...
    afterEach(() => {
      ...
      test('should create offer', async () => {
        ...
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        expect(mockInstance.createOffer).toHaveBeenCalled();
        expect(mockInstance.setLocalDescription).toHaveBeenCalledWith(offer);
      });
      ...
      test('should create answer', async () => {
        ...
        // Get the mock instance that was created
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        expect(mockInstance.createAnswer).toHaveBeenCalled();
        expect(mockInstance.setLocalDescription).toHaveBeenCalledWith(answer);
      });
      ...
      test('should set remote description', async () => {
        ...
        // Get the mock instance that was created
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        expect(mockInstance.setRemoteDescription).toHaveBeenCalledWith(description);
      });
      ...
      test('should add ICE candidate', async () => {
        ...
        // Get the mock instance that was created
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        expect(mockInstance.addIceCandidate).toHaveBeenCalledWith(candidate);
      });
      ...
      test('should create data channel', async () => {
        ...
        // Get the mock instance that was created
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        expect(mockInstance.createDataChannel).toHaveBeenCalledWith('test-channel', {
          ordered: true,
      ...
      test('should send data through channel', async () => {
        ...
        // Get the mock instance and then the data channel
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        const mockDataChannel = mockInstance.createDataChannel.mock.results[0].value;
    ...
    describe('Connection Statistics', () => {
      ...
      test('should close connection', async () => {
        ...
        // Get the mock instance and verify close was called
        const mockInstance = jest.mocked(RTCPeerConnection).mock.results[0].value;
        const mockDataChannel = mockInstance.createDataChannel.mock.results[0].value;
    ...
    let mockSocket: any;    beforeEach(() => {
      // Setup mock socket with proper Jest mocks
      mockSocket = {
        connect: jest.fn(),
        disconnect: jest.fn(),
        emit: jest.fn(),
    ...
    afterEach(() => {
      ...
      test('should connect to signaling server', async () => {
        ...
        expect(mockedIo).toHaveBeenCalledWith('ws://localhost:3002', expect.objectContaining({
      ...
      test('should discover devices', async () => {
        // First connect to set up the socket and event handlers
        const connectPromise = signalingClient.connect();
      ...
      test('should send connection request', async () => {
        ...
        // Now test connection request
        const requestPromise = signalingClient.requestConnection('device-456');
        
        // Simulate acceptance response
        process.nextTick(() => {
          // Find the signaling-message handler that was set up during connect
          const messageHandler = mockSocket.on.mock.calls.find(call => call[0] === 'signaling-message');
          if (messageHandler) {
            // Find the emit call that sent the connection-request
            const requestCall = mockSocket.emit.mock.calls.find(call => 
              call[0] === 'signaling-message' && 
              call[1]?.type === 'connection-request'
            );
            const requestId = requestCall?.[1]?.data?.requestId;
      ...
      test('should handle connection rejection', async () => {
        ...
        process.nextTick(() => {
          ...
          if (messageHandler) {
            ...
            // Simulate the rejection response
            messageHandler[1]({
              type: 'connection-response',
              from: 'device-456',
              data: {
