...
export class ConnectionManager extends EventEmitter {
  ...
  constructor(
    ...
    super();
    this.deviceId = deviceId; // Initialize logger with better fallback handling
  ...
  async handleConnectionLoss(connectionId: string): Promise<void> {
    ...
    this.logger.warn("Connection lost detected", {
      connectionId,
  ...
  async attemptReconnection(connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.reconnectDelay * (attempts + 1))
      );
      ...
      const newConnectionId = await this.webrtcManager.createPeerConnection(
        connection.deviceId
      );
  ...
  async discoverDevices(timeout: number = 5000): Promise<DiscoveryResult[]> {
    try {
      ...
      throw new NetworkError("Device discovery failed", "DISCOVERY_FAILED", {
        error,
      });
    }
  }
  ...
  async updateQualitySettings(connectionId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.debug("Quality settings updated", {
        connectionId,
        qualitySettings,
      });
  ...
  getConnectionStatistics(connectionId: string): any {
    ...
    const webrtcStats =
      this.webrtcManager.getConnectionStatistics(connectionId);
    return {
  ...
  private async performConnectionHandshake(
    ...
    try {
      ...
      const response = await this.signalingClient.requestConnection(
        targetDeviceId,
        {
          offer,
          options,
      ...
      if (!response.accepted) {
        throw new NetworkError("Connection rejected", "CONNECTION_REJECTED", {
          response,
        });
      }
      ...
      if (response.answer) {
        await this.webrtcManager.setRemoteDescription(
          connectionId,
          response.answer
        );
      }

      this.logger.debug("Connection handshake completed", {
        connectionId,
        targetDeviceId,
      });
    } catch (error) {
  ...
  private handleWebRTCStateChange(event: {
    connectionId: string;
    deviceId: string;
  ...
  private async handleIceCandidate(event: {
    connectionId: string;
    deviceId: string;
  ...
  private async handleIncomingConnectionRequest(
    request: ConnectionRequest
  ): Promise<void> {
    try {
      this.logger.info("Incoming connection request", {
        from: request.from,
      ...
      await this.signalingClient.respondToConnection(
        request,
        true,
        undefined,
        answer
      ...
      this.logger.info("Incoming connection accepted", {
        from: request.from,
        connectionId,
      });
    } catch (error) {
  ...
  private initializeLogger(): any {
    try {
      ...
      if (typeof Logger !== "undefined" && Logger.createLogger) {
        const logger = Logger.createLogger("ConnectionManager");
        if (logger) return logger;
      }
    } catch (error) {
      // Logger might not be available or might throw
      console.warn("Logger creation failed:", error);
    }
    ...
    console.warn("Using fallback logger in ConnectionManager");
